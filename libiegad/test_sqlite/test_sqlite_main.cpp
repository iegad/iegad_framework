...
std::string DROP_TAB_SQL = "DROP TABLE TEST_TAB";
...
main(int argc, char * argv[])
{
	...
	if (db_handler.exec(DROP_TAB_SQL) != 0) {
	    break;
	}
	...
	char c = 'a';
	for (int i = 0; i < 20; i++) {
	    sqlvct.push_back(std::move("INSERT INTO TEST_TAB (T_ID, T_VAL) VALUES (" + std::to_string(i) + ", '" + std::to_string((char)c++) + "')"));
	}
    ...
    std::cout << db_handler.errmsg() << std::endl;
    std::cin.get();
