...
iegad::netc::udp_puller::udp_puller(const std::string & ipstr, unsigned short port, callback_t callback)
    :
    callback_(callback),
    ios_(), 
    sock_(ios_)
...
int 
iegad::netc::udp_puller::start()
{
    thread_proc_ = std::move(std::thread(std::bind(&iegad::netc::udp_puller::_start, this)));
    thread_proc_.detach();
    return 0;
}
...
iegad::netc::udp_puller::~udp_puller()
{
    sock_.close();
}
