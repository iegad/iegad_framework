...
TEST(STR_UTEST_CASE, ENCODE_TEST)
{
    ...
    std::string hexstr = iegad::string::bin_tostr(srcString.c_str(), srcString.size());
    len = 1024;
    iegad::string::str_tobin(hexstr, buff, len);
    std::string rzt = std::string(buff, len);
    EXPECT_EQ("Hello world", rzt);
}
